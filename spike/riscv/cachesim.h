...
class cache_sim_t
{
 ...
 protected:
  ...
  virtual uint64_t LRU(uint64_t addr);
  virtual uint64_t FIFO(uint64_t addr); // added these 2 functions to use in cachesim.cc
  ...
  uint64_t FIFOCount;		
  /*
   * LRUCount = a counter to check when is the address accessed.
   * FIFOCount = a counter to check when is the address added to cache.
   * LRUTime, FIFOTime = a storage to store LRUCounts, using its index in tags
   */
